...
Gabbia Sferica Ibrida a 24 Gruppi - Architettura Fibonacci a Spinta Vettoriale Accumulata
Legge di Fase Progressiva Unidirezionale (Pisano mod 9 + Sfasamento Angolare Sincrono Delta_theta = 15°).
Potenza Calibrata: 100 W per Solenoide (Potenza Totale Array Solenoidi: 2.40 kW).

Caratteristiche della simulazione:
- 24 gruppi di solenoidi distribuiti sull'equatore a Delta_theta = 15° (576 nodi gerarchici equivalenti)
- Legge di fase progressiva unidirezionale:
    phi_k = (v_k / 9) * 2*pi + k * Delta_theta_prog
  con rimozione dell'auto-cancellazione diametrale per massimizzare la spinta netta accumulata
...
- Generazione Tavola Diagnostica 300 DPI (Figura 28)
...
BASE_SIF = CONFIG_DIR / "case_fibonacci_24x24_spinta_accumulata.sif"
MESH_DIR = VAR_DIR / "mesh"
MESH_NAME = "macchina_fibonacci_24x24"
WORK_DIR = VAR_DIR / "work_dirs" / "run_fibonacci_spinta_accumulata"
RES_DIR = WORK_DIR / "results"
DATA_DIR = VAR_DIR / "data"
DATA_DIR.mkdir(parents=True, exist_ok=True)
OUT_JSON = DATA_DIR / "fibonacci_24x24_spinta_unidirezionale.json"

FIG_VAR_DIR = VAR_DIR / "figures"
FIG_VAR_DIR.mkdir(parents=True, exist_ok=True)
FIG_28_NAME = "fig_28_fibonacci_24x24_spinta_accumulata.png"
VIDEO_NAME = "video_dinamica_fibonacci_spinta_accumulata.gif"
...
DELTA_TH_PROG = 2.0 * np.pi / 24.0  # 15°
PHASES_RAD = [((r / 9.0) * 2.0 * np.pi + k * DELTA_TH_PROG) % (2.0 * np.pi) for k, r in enumerate(PISANO_ROOTS)]
...
def prepare_and_run_solver():
    ...
    vtus = sorted(list(RES_DIR.glob("macchina_fibonacci_accumulata_out_t*.vtu")))
    ...
    print("=" * 80)
    print("  [SOLVER] Esecuzione Elmer FEM 3D: Fibonacci 24x24 Spinta Accumulata (100 W/bobina)")
    print(f"  [DIR] Work Dir: {WORK_DIR}")
    print("=" * 80)

    for f in RES_DIR.glob("macchina_fibonacci_accumulata_out_t*.vtu"):
        f.unlink()
...
def process_simulation_data(vtus):
    ...
    data_summary = {
        "architecture": {
            "name": "Gabbia Sferica Ibrida a 24 Gruppi - Spinta Vettoriale Accumulata",
            "n_groups": 24,
            "delta_theta_deg": 15.0,
            "hierarchical_nodes": 576,
            "pisano_period": 24,
            "pisano_mod": 9,
            "delta_theta_prog_deg": 15.0,
            "roots": PISANO_ROOTS,
...
def generate_figure_28(data, time_s, forces, joule_eddy, per_coil_powers):
    """Genera la Tavola Diagnostica a 300 DPI per la Spinta Accumulata (Figura 28)."""
    print("=" * 80)
    print("  [PLOTTING] Generazione Figura 28: Diagnostica Spinta Accumulata (300 DPI)")
    print("=" * 80)
    ...
    # ----------------------------------------------------
    # PANNELLO A: Mappa dei 24 Settori con Fase Progressiva
    # ----------------------------------------------------
    ax_a = fig.add_subplot(gs[0, 0], projection='polar')

    angles = np.linspace(0, 2.0 * np.pi, 24, endpoint=False)
    phases_deg = data["architecture"]["phases_deg"]
    roots = data["architecture"]["roots"]

    norm_phases = np.array(phases_deg) / 360.0
    colors = plt.cm.viridis(norm_phases)

    bars = ax_a.bar(angles, np.array(phases_deg), width=2.0*np.pi/24.0*0.88, bottom=0.0,
    ...
    for k in range(24):
        ...
        ph_val = phases_deg[k]
        r_val = roots[k]
        ax_a.text(ang, ph_val + 25.0, f"B{k+1}\n({r_val})", ha='center', va='center',
                  fontsize=8, fontweight='bold', color='#1a1a2e')

    # Linee di asimmetria diametrale (evidenziando Delta_phi != 0, 180)
    for k in range(12):
        opp_k = k + 12
        ax_a.plot([angles[k], angles[opp_k]], [phases_deg[k], phases_deg[opp_k]],
                  color='#d90429', linestyle='--', linewidth=0.9, alpha=0.5)

    ax_a.set_title(r"PANEL A: Mappa Fase Progressiva Unidirezionale ($\Delta\theta_{\mathrm{prog}} = 15^\circ$)" + "\n" +
                   r"Rottura dell'Auto-Cancellazione Diametrale: $\phi_{k+12} \neq -\phi_k$ (Spinta Accumulata)",
                   fontsize=11, fontweight='bold', pad=15)
    ax_a.set_rticks([90, 180, 270, 360])
    ax_a.set_rlabel_position(45)
    ax_a.set_yticklabels(['90°', '180°', '270°', '360°'], fontsize=8)
    ax_a.grid(True, linestyle='--', alpha=0.5)

    # ----------------------------------------------------
    # PANNELLO B: Dinamica delle Forze di Lorentz Accumulate
    # ----------------------------------------------------
    ...
    mean_fx_uN = data["thrust_and_forces"]["mean_fx_uN"]
    mean_fy_uN = data["thrust_and_forces"]["mean_fy_uN"]
    mean_fz_uN = data["thrust_and_forces"]["mean_fz_uN"]

    ax_b.axhline(mean_f_uN, color='#6c757d', linestyle=':', linewidth=1.5,
                 label=f"Media Stazionaria = {mean_f_uN:.2f} uN")

    ax_b.set_title(f"PANEL B: Waveform delle Forze di Lorentz Accumulate\nSpinta Risultante Media: |F| = {mean_f_uN:.2f} uN (Picco: {peak_f_uN:.2f} uN)",
...
def generate_animated_video(vtus, data, time_s, forces):
    ...
    """
    print("=" * 80)
    print("  [VIDEO] Generazione Video Dinamico 3D di Spinta Accumulata")
    print("=" * 80)
    ...
    for idx, vtu_file in enumerate(vtus):
        ...
        b_mag_grid = np.linalg.norm(b_grid, axis=1).reshape(XI.shape) * 1000.0
...
def main():
    print("=" * 80)
    print("CAMPAIGN RUNNER: FIBONACCI 24x24 A SPINTA VETTORIALE ACCUMULATA (100 W/BOBINA)")
    print("=" * 80)

    # 1. Esecuzione simulatore
    ...
    # 3. Generazione Figura 28 a 300 DPI
    generate_figure_28(data_summary, time_s, forces, joule_eddy, per_coil_powers)
    ...
    print(f"  - Figura 28 (300 DPI): {ROOT_FIGURES_DIR / FIG_28_NAME}")
