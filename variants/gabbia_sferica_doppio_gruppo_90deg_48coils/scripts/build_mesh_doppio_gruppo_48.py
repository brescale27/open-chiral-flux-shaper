...
Gabbia Sferica con Doppio Macro-Gruppo Ortogonale a 90° (48 Solenoidi Totali)
- Gruppo 1 (Equatoriale Z=0): 24 solenoidi paralleli all'asse Z (R_c1 = 37 mm)
- Gruppo 2 (Meridiano X=0): 24 solenoidi paralleli all'asse X (R_c2 = 31 mm)
...
MSH_FILE = MESH_DIR / "macchina_doppio_gruppo_48.msh"
OUT_MESH_DIR = MESH_DIR / "macchina_doppio_gruppo_48"
def find_elmergrid():
...
def build():
    print("=" * 80)
    print("BUILD MESH 3D: DOPPIO MACRO-GRUPPO ORTOGONALE A 90° (48 SOLENOIDI)")
    print("=" * 80)

    MESH_DIR.mkdir(parents=True, exist_ok=True)

    gmsh.initialize()
    gmsh.model.add("macchina_doppio_gruppo_48")
    occ = gmsh.model.occ
    ...
    rc2 = 0.031      # 31 mm raggio cerchio Gruppo 2 (X)
    r_wire = 0.0025  # 2.5 mm raggio filo solenoide
    h_wire = 0.012   # 12 mm altezza solenoide
    ...
    # Gruppo 1: 24 solenoidi nel piano equatoriale XY (paralleli a Z)
    coils_1 = []
    for k in range(24):
        ang = k * (2 * np.pi / 24)
        x = rc1 * np.cos(ang)
    ...
    # Gruppo 2: 24 solenoidi nel piano meridiano YZ (paralleli a X)
    coils_2 = []
    for k in range(24):
        ang = k * (2 * np.pi / 24)
        y = rc2 * np.cos(ang)
    ...
    print(f"Creati {len(coils_1)} solenoidi nel Gruppo 1 (Z) e {len(coils_2)} nel Gruppo 2 (X). Totale: 48")
    ...
    # Classificazione dei corpi fisici 3D
    ...
    for dim, tag in entities:
        ...
        elif abs(com[2]) < 0.005 and 0.034 < rcom < 0.040:
            classification['coils1'].append(tag)
        elif abs(com[0]) < 0.005 and 0.028 < rcom < 0.034:
    ...
    if len(classification['coils1']) != 24:
        print(f"[ERRORE] Trovate {len(classification['coils1'])} bobine nel Gruppo 1 anziché 24!")
        sys.exit(1)
    if len(classification['coils2']) != 24:
        print(f"[ERRORE] Trovate {len(classification['coils2'])} bobine nel Gruppo 2 anziché 24!")
        sys.exit(1)
